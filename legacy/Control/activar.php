<?php
    if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
        ...
        $search = mysql_query("SELECT email, hash_, usuarioValido FROM cliente WHERE email_clie='".$email."' AND hash_='".$hash."' AND usuarioValido='0'") or die(mysql_error()); 
        $match  = mysql_num_rows($search);
                    
        if($match > 0){
            // Hay una coincidencia, activar la cuenta
            mysql_query("UPDATE usuario SET usuarioValido='1' WHERE email='".$email."' AND hash_='".$hash."' AND usuarioValido='0'") or die(mysql_error());
